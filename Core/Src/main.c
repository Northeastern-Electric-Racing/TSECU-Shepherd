...
void print_bms_stats(acc_data_t *acc_data)
{
	...
	printf("Min, Max, Avg Temps: %ld, %ld, %d\n", acc_data->min_temp.val,
	       acc_data->max_temp.val, acc_data->avg_temp);
#endif

#ifdef DEBUG_VOLTAGES
	printf("Min, Max, Avg, Delta Voltages: %f, %f, %f, %f\n",
	       acc_data->min_voltage.val, acc_data->max_voltage.val,
	       acc_data->avg_voltage, acc_data->delt_voltage);
#endif
	...
	for (uint8_t c = 0; c < NUM_CHIPS; c++) {
		...
		for (uint8_t cell = 0; cell < num_cells; cell++) {
			printf("%.2f\t",
			       acc_data->chip_data[c].cell_voltages[cell]);
		}
		printf("\n");
	}
#endif
	...
	for (uint8_t c = 0; c < NUM_CHIPS; c++) {
		...
		for (uint8_t cell = 0; cell < num_cells; cell++) {
			printf("%.5f\t",
			       getVoltage(
				       acc_data->chips[c].cell.c_codes[cell]));
		}
		printf("\n");
	}
...
#define DEBUG_THERM_VOLTS
	...
	for (uint8_t c = 0; c < NUM_CHIPS; c++) {
		for (uint8_t gpio = 0; gpio < 10; gpio++) {
			printf("%f\t",
			       getVoltage(
				       acc_data->chips[c].raux.ra_codes[gpio]));
		}
		printf("\n");
	}
	printf("THERM TEMPS: \n");
	for (uint8_t c = 0; c < NUM_CHIPS; c++) {
		...
		for (uint8_t cell = 0; cell < num_cells; cell++) {
			printf("%.1f\t",
			       acc_data->chip_data[c].cell_temp[cell]);
		}
		printf("\n");
	}
	printf("CHIP TEMPS: \n");
...
static void MX_GPIO_Init(void)
{
	...
	HAL_GPIO_WritePin(GPIOC,
			  EXT_GPIO_1_Pin | EXT_GPIO_5_Pin | EXT_GPIO_4_Pin |
				  SPI3_CS_Pin | SPI2_CS_Pin | DEBUG_LED_2_Pin,
			  GPIO_PIN_RESET);
	...
	HAL_GPIO_WritePin(GPIOB,
			  FAULT_MCU_Pin | DEBUG_LED_1_Pin | WATCHDOG_Pin |
				  EXT_GPIO_2_Pin | EXT_GPIO_0_Pin,
			  GPIO_PIN_RESET);

	/*Configure GPIO pins : EXT_GPIO_1_Pin EXT_GPIO_5_Pin EXT_GPIO_4_Pin SPI3_CS_Pin
                           SPI2_CS_Pin DEBUG_LED_2_Pin */
	GPIO_InitStruct.Pin = EXT_GPIO_1_Pin | EXT_GPIO_5_Pin | EXT_GPIO_4_Pin |
			      SPI3_CS_Pin | SPI2_CS_Pin | DEBUG_LED_2_Pin;
	...
	/*Configure GPIO pins : I_SENSE_0_Pin SPI1_CS_Pin */
	GPIO_InitStruct.Pin = I_SENSE_0_Pin | SPI1_CS_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	...
	/*Configure GPIO pins : FAULT_MCU_Pin DEBUG_LED_1_Pin WATCHDOG_Pin EXT_GPIO_2_Pin
                           EXT_GPIO_0_Pin */
	GPIO_InitStruct.Pin = FAULT_MCU_Pin | DEBUG_LED_1_Pin | WATCHDOG_Pin |
			      EXT_GPIO_2_Pin | EXT_GPIO_0_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
