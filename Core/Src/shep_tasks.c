...
const void print_bms_stats(analyzer_t *analyzer, hv_plate_t *hv_plate,
  ...
  for (uint8_t c = 0; c < NUM_CHIPS; c++) {
    for (uint8_t cell = 0; cell < NUM_CELLS_PER_CHIP; cell++) {
      PRINTLN_INFO("%.2f\t", analyzer->chip_data[c].cell_voltages[cell]);
    }
    printf("\n");
  }
  ...
  for (uint8_t c = 0; c < NUM_CHIPS; c++) {
    for (uint8_t cell = 0; cell < NUM_CELLS_PER_CHIP; cell++) {
      PRINTLN_INFO("%.2f\t", analyzer->chip_data[c].open_cell_voltage[cell]);
    }
    printf("\n");
  }
#endif
  ...
  for (uint8_t c = 0; c < NUM_CHIPS; c++) {
    for (uint8_t cell = 0; cell < NUM_CELLS_PER_CHIP; cell++) {
      PRINTLN_INFO("%.1f\t", analyzer->chip_data[c].cell_temp[cell]);
    }
    printf("\n");
  }
...
void vDefaultTask(ULONG thread_input) {
  default_task_args_t *default_task_args = (default_task_args_t *)thread_input;
...
void vEthernetIncoming(ULONG thread_input) {
  while (1) {
    ...
    /* Process incoming messages */
    while (queue_receive(&eth_incoming, &message, TX_WAIT_FOREVER) ==
           U_SUCCESS) {
...
void vEthernetOutgoing(ULONG thread_input) {
  while (1) {
    ...
    while (queue_receive(&eth_outgoing, &message, TX_WAIT_FOREVER) ==
           U_SUCCESS) {
      status = ethernet_send_message(&message);
      if (status != U_SUCCESS) {
        PRINTLN_WARNING("Failed to send Ethernet message after removing from "
...
void vGetSegmentData(ULONG thread_input) {
  ...
  for (;;) {
    ...
    if (current_state == CHARGING || current_state == BALANCING) {
      ...
      isospi_handle_state(acc_data->chips, state_machine, &hspi2);
      ...
      if (DEBUG_MODE_ENABLED) {
        segment_retrieve_debug_data(acc_data->chips, &hspi2);
      }
    }
    ...
    if (get_current_state(state_machine) == BALANCING) {
      segment_configure_balancing(acc_data->chips, acc_data->discharge_config,
...
void vHvPlateData(ULONG thread_input) {
  ...
  for (;;) {
    ...
    /* Check whether pulse operation needs to be disabled due to charging state
     * or faults */
...
void vPrecharge(ULONG args) {
  ...
  precharge_init(&precharge_config, hv_plate, 0.9f, 200 /* ms debounce time */);
...
void vPeripherals(ULONG thread_input) {
  ...
  for (;;) {
    ...
    tempsensor_getTemperatureAndHumdidty(peripherals, &peripherals->sht30.temp,
                                         &peripherals->sht30.humidity);

    PRINTLN_INFO("SHT30 Temp Sensor Reading: %.2f C", peripherals->sht30.temp);
    PRINTLN_INFO("SHT30 Temp Humidity Sensor Reading: %.2f %%",
                 peripherals->sht30.humidity);
    ...
    for (uint8_t chip = 0; chip < NUM_CHIPS; chip++) {
      ...
      for (uint8_t cell = 0; cell < NUM_CELLS_PER_CHIP; cell += 2) {
        ...
        send_cell_data_message(
            chip_data->alpha, chip_data->cell_temp[cell],
            chip_data->cell_voltages[cell],

            cell + 1 == NUM_CELLS_PER_CHIP ? 0
                                           : chip_data->cell_voltages[cell + 1],
            chip, cell, cell + 1, chip_data->is_balancing[cell],
      ...
      send_status_a_message(chip, chip_data->die_temp, chip_data->vpv,
                            chip_data->vmv, &chip_data->flt_reg);

      tx_thread_sleep(30); // TODO: enhance timing

      send_status_b_message(chip_data->v_res, chip, chip_data->vref2,
                            chip_data->v_analog, chip_data->v_digital,
                            &chip_data->flt_reg);
...
uint8_t shep_threads_init(TX_BYTE_POOL *byte_pool) {
  ...
  CATCH_ERROR(create_thread(byte_pool, &_ethernet_incoming_thread), U_SUCCESS);
  CATCH_ERROR(create_thread(byte_pool, &_ethernet_outgoing_thread), U_SUCCESS);
