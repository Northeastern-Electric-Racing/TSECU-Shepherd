...
// init functions
void init_boot(state_machine_t *state_machine);
void init_ready(state_machine_t *state_machine);
void init_charging(state_machine_t *state_machine);
void init_balancing(state_machine_t *state_machine);
void init_faulted(state_machine_t *state_machine);

// handle functions
void handle_boot(state_machine_t *state_machine);
void handle_ready(state_machine_t *state_machine);
void handle_charging(state_machine_t *state_machine);
void handle_balancing(state_machine_t *state_machine);
void handle_faulted(state_machine_t *state_machine);

void request_transition(state_machine_t *state_machine, state_t next_state);

typedef void (*HandlerFunction_t)(state_machine_t *state_machine);
typedef void (*InitFunction_t)(state_machine_t *state_machine);
...
void handle_boot(state_machine_t *state_machine)
{
	request_transition(state_machine, READY);
	return;
}
...
void init_charging(state_machine_t *state_machine)
{
	cancel_timer(&state_machine->charger_settle_countup_timer);
	return;
}
...
void handle_balancing(state_machine_t *state_machine) {
	if (sm_balancing_check(state_machine)) {
		handle_balance_cells(state_machine->analyzer_data);
	} else {
		request_transition(state_machine, CHARGING);
	}
	return;
}
...
void handle_charging(state_machine_t *state_machine)
{
	...
	if (sm_charging_check(state_machine)) {

		/* Send CAN message, but not too often */
		if (is_timer_expired(&state_machine->charger_message_timer) ||
		    !is_timer_active(&state_machine->charger_message_timer)) {
			send_charging_message((MAX_CHARGE_VOLT *
					       (NUM_CELLS_PER_CHIP * 2) *
					       NUM_SEGMENTS),
					      CHARGING_CURRENT, true);
			start_timer(&state_machine->charger_message_timer, 1000);
		}
	} else {
	...
	if (sm_balancing_check(state_machine))
		request_transition(state_machine, BALANCING);
...
void init_faulted(state_machine_t *bmsdata)
{
	send_mc_charge_message(0);
	send_mc_discharge_message(0);	
	send_charging_message(0, 0, false);
...
void handle_faulted(state_machine_t *state_machine)
{
	...
	if (state_machine->fault_code_crit == FAULTS_CLEAR) {
		compute_set_fault(false);
		request_transition(state_machine, BOOT);
		return;
	}
}
...
void sm_handle_state(state_machine_t *state_machine)
{
	...
	sm_fault_return(state_machine);

	if (state_machine->fault_code_crit != FAULTS_CLEAR) {
		request_transition(state_machine, FAULTED);
	}

	handler_LUT[state_machine->bms_state](state_machine);
}
...
void request_transition(state_machine_t *state_machine, state_t next_state)
{
	...
	init_LUT[next_state](state_machine);
...
void sm_fault_return(state_machine_t *state_machine)
{
	...
	analyzer_t *fault_data = state_machine->analyzer_data;
	...
	fault_table[0]  = (fault_eval_t) {.id = "Discharge Current Limit", .timer =       ovr_curr_timer, .data_1 =     fault_data->hv_plate->pack_current,  .optype_1 = GT, .lim_1 = fault_data->cont_DCL ,                                                .timeout =      OVER_CURR_TIME, .code = DISCHARGE_LIMIT_ENFORCEMENT_FAULT,  .optype_2 = NOP/* ------------------------------UNUSED-------------------------*/, .is_critical = true  };
	fault_table[1]  = (fault_eval_t) {.id = "Charge Current Limit",    .timer =    ovr_chgcurr_timer, .data_1 =     fault_data->hv_plate->pack_current,  .optype_1 = GT, .lim_1 =                                        fault_data->cont_CCL,          .timeout =  OVER_CHG_CURR_TIME, .code =    CHARGE_LIMIT_ENFORCEMENT_FAULT,  .optype_2 = LT,  .data_2 =         fault_data->hv_plate->pack_current,  .lim_2 =          0, .is_critical = true  };
	...
	fault_table[3]  = (fault_eval_t) {.id = "High Charge Voltage",     .timer =    ovr_chgvolt_timer, .data_1 =  fault_data->max_ocv.val,      .optype_1 = GT, .lim_1 =                                              MAX_CHARGE_VOLT,         .timeout =  OVER_VOLT_TIME,     .code =             CELL_VOLTAGE_TOO_HIGH,  .optype_2 = EQ, .data_2 = state_machine->bms_state == CHARGING,  .lim_2 =      true,   .is_critical = true  };
	...
	for (int i = 0; i < NUM_FAULTS; i++) {
		...
		if (status == FAULT_STAT_FAULTED) {
			if (fault_table[i].is_critical) {
				state_machine->fault_code_crit |= item_code;
			} else {
				state_machine->fault_code_noncrit |= item_code;
			}
...
bool sm_charging_check(state_machine_t *state_machine)
{
	...
	if (!is_timer_expired(&state_machine) &&
	    is_timer_active(&state_machine->charge_settle_countdown_timer)) {
		//printf("Charger settle countup active\r\n");
		return false;
	}

	// if we are counting down (the normal charging time)
	if (is_timer_active(&state_machine->charge_settle_countdown_timer)) {
		// if we need to stop charging, start the pause timer and stop charging immediately
		if (is_timer_expired(&state_machine->charge_settle_countdown_timer)) {
			start_timer(&state_machine->charge_settle_countdown_timer,
				    CHARGE_SETL_TIMEOUT);
			return false;
		} else
			return true;
		...
		start_timer(&state_machine->charge_settle_countdown_timer, CHARGE_SETL_TIMEUP);
		return true;
	}
}
...
bool sm_balancing_check(state_machine_t *state_machine)
{
	...
	if (state_machine->analyzer_data->max_voltage.val <= BAL_MIN_V)
		return false;
	if (state_machine->analyzer_data->delt_voltage <= MAX_DELTA_V)
		return false;
