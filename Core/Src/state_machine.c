...
const InitFunction_t init_LUT[NUM_STATES] = {
    &init_boot, &init_ready, &init_charging, &init_balancing, &init_faulted};

const HandlerFunction_t handler_LUT[NUM_STATES] = {
    &handle_boot, &handle_ready, &handle_charging, &handle_balancing,
    &handle_faulted};
...
void handle_ready(state_machine_args_t *state_machine_args) { return; }
...
void handle_charging(state_machine_args_t *state_machine_args) {
  ...
  if (sm_charging_check(state_machine_args)) {
    ...
    if (is_timer_expired(
            &state_machine_args->state_machine->charger_message_timer) ||
        !is_timer_active(
            &state_machine_args->state_machine->charger_message_timer)) {
      send_charging_message(
          (MAX_CHARGE_VOLT * (NUM_CELLS_PER_CHIP * 2) * NUM_SEGMENTS),
          CHARGING_CURRENT, true);
      start_timer(&state_machine_args->state_machine->charger_message_timer,
                  1000);
    }
  } else {
...
bool sm_fault_eval(fault_eval_t *item) {
  ...
  bool fault_present =
      (condition1 && condition2) || (condition1 && item->optype_2 == NOP);
...
void vStateMachine(ULONG thread_input) {
  ...
  for (;;) {
    ...
    if (is_timer_expired(&telem_timer)) {
      analyzer->internal_temp = peripherals->board_temp.temp_c;
