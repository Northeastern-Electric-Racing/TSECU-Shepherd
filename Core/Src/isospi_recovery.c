 ...
 */
static uint8_t isospi_verify_recovery(cell_asic chips[NUM_CHIPS],
				      uint8_t start_chip_idx)
{
	uint8_t result = 0U;

	for (uint8_t i = start_chip_idx; i < NUM_CHIPS; i++) {
		if (chips[i].pec_error_sum > ISOSPI_VALIDATION_THRESHOLD) {
 ...
 */
static void isospi_detect_break(cell_asic chips[NUM_CHIPS],
		...
		if (is_timer_expired(&pec_accum_timer)) {
			...
			if (fault_detected == 1U) {
				...
				for (uint8_t chip = first_faulty_chip_idx; chip < NUM_CHIPS; chip++) {
					if (chips[chip].pec_error_sum <= ISOSPI_PEC_ERROR_THRESHOLD) {
						all_above_thresh = 0U;
						break;
					}
				}
 ...
 */
static void isospi_recover_break(cell_asic chips[NUM_CHIPS],
...
void isospi_break_detection_init(cell_asic chips[NUM_CHIPS])
{
	...
	isospi_status.break_chip = 0U;
	isospi_status.verification_attempts = 0U;
	isospi_status.recovery_successful = 0U;
	isospi_status.fault_latched = 0U;

	reset_all_pec_error_sums(chips);

	// TDOD send_isospi_status_message(&isospi_status);
}
			 ...
			 state_machine_t *state_mach, SPI_HandleTypeDef *hspi)
{
	...
	case ISOSPI_BREAK_DETECTED:
		// TODO send_isospi_status_message(&isospi_status);
