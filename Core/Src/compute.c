...
static int32_t _p3t1755_read(uint16_t dev_addr, uint16_t reg, uint8_t *data,
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Master_Receive(&hi2c1, dev_addr, data, length, HAL_MAX_DELAY);
...
static int32_t _p3t1755_write(uint16_t dev_addr, uint16_t reg, uint8_t *data,
                              uint8_t length) {
  HAL_StatusTypeDef status =
      HAL_I2C_Master_Transmit(&hi2c1, dev_addr, data, length, HAL_MAX_DELAY);
  if (status != HAL_OK) {
...
static int32_t _lsm6dsv_read(void *spi_handle, uint8_t reg, uint8_t *buffer,
  ...
  uint8_t spi_reg =
      (uint8_t)(reg | 0b10000000); // Bits 0 through 6 store 'reg' (the register
...
static int32_t _lsm6dsv_write(void *spi_handle, uint8_t reg,
  ...
  uint8_t spi_reg =
      (uint8_t)(reg & 0b01111111); // Bits 0 through 6 store 'reg' (the register
  // address), while Bit 7 lets you chose if it's a
  // read or write operation (1=read, 0=write).
  status = HAL_SPI_Transmit(handle, &spi_reg, sizeof(spi_reg), HAL_MAX_DELAY);
  if (status != HAL_OK) {
