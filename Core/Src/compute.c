...
static int32_t _p3t1755_read(uint16_t dev_addr, uint16_t reg, uint16_t *data) {
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Master_Receive(&hi2c1, dev_addr, data, sizeof(data) / *data,
...
static int32_t _p3t1755_write(uint16_t dev_addr, uint16_t reg, uint16_t *data) {
  HAL_StatusTypeDef status =
      HAL_I2C_Master_Transmit(&hi2c1, dev_addr, data, sizeof(data) / *data,
                              p3t1755_110MS_CONVERSION_TIME);
  if (status != HAL_OK) {
    PRINTLN_ERROR("Failed to call HAL_I2C_Master_Transmit() to write to "
                  "P3T1755 (Status: %d/%s).",
                  status, hal_status_toString(status));
    return status;
  }
...
static int32_t _lsm6dsv_read(void *spi_handle, uint8_t reg, uint8_t *buffer,
  ...
  if (status != HAL_OK) {
    PRINTLN_ERROR("Failed to call HAL_SPI_Transmit() to write the first SPI "
                  "command (Status: %d/%s).",
                  status, hal_status_toString(status));
...
static const p3t1755_t p3t = {P3T1755_DEV_ADDR, _p3t1755_write, _p3t1755_read};
int p3t_init(void) {
  ...
  int status = p3t1755_configure(&p3t, 0, 1, 0, defalt_two_bit, defalt_two_bit);
  if (status != 0) {
    PRINTLN_ERROR(
...
static const stmdev_ctx_t imu = {
    .handle = &hspi6, .read_reg = _lsm6dsv_read, .write_reg = _lsm6dsv_write};
